...
Muestra exactamente qué detecta YOLO, por qué cada objeto pasa o no
...
WEIGHTS = os.getenv("YOLO_WEIGHTS", "yolov8x.pt")
...
def run(image_path: str, threshold_endpoint: float):
    print(f"\n{SEP2}")
    print(f"  DIAGNOSTICO YOLO — {image_path}")
    ...
    for d in passed:
        ...
        print(f"    > {label:<20} {conf:.1%}  zona={dep}_{col}  tamano={area:.3f}")

    # ── Objetos navegables perdidos ────────────────────────────
    lost = [d for d in all_dets if d not in passed and d["label"] in _NAV_CLASSES]
    if lost:
        print(f"\n  WARN  {len(lost)} objeto(s) de navegacion que NO pasan:\n")
        for d in lost:
            label     = d["label"]
            conf      = d["conf"]
            class_min = _CLASS_MIN_CONF.get(label, threshold_endpoint)
            effective = min(class_min, threshold_endpoint)
            print(f"    x {label:<20} conf={conf:.1%}  "
    ...
    if not passed:
        print("  ERROR: YOLO no detecto ningun objeto relevante.")
        print("  Verifica YOLO_WEIGHTS=yolov8x.pt y YOLO_IMGSZ=1280 en .env")
    elif len(passed) < 3:
    ...
    if lost:
        ...
        print("  Futuro: YOLOv8-World para vocabulario abierto sin limitaciones COCO")
